...
| Date | Task | Files Changed | Env Vars | DB Migration | Redeploy | Rollback |
|------|------|--------------|----------|-------------|----------|----------|
| 2026-06-11 | Record Painting References progress and define Phase 1B finished-painting upload roadmap | docs/WCWIKI_FUTURE_PROOF_EXPANSION_ROADMAP.md, docs/TASK_LOG.md | None | None | No | Revert this documentation commit |
| 2026-06-11 | Pass OpenAI/log environment variables into production app container and document admin log note | docker-compose.prod.yml, src/app/[locale]/admin/api-keys/page.tsx, docs/DEPLOYMENT_WORKFLOW.md, docs/TASK_LOG.md | OPENAI_API_KEY, OPENAI_VISION_MODEL, OPENAI_METADATA_DAILY_LIMIT, optional APP_LOG_DIR now passed through compose | None | Yes | Revert this commit; ensure any removed env vars are not needed by deployed AI metadata suggestions |
