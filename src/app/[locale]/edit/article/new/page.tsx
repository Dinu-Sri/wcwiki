...
export default function NewArticlePage() {
  return (
    <Suspense fallback={<><Header /><main className="min-h-screen flex items-center justify-center text-muted">Loading...</main><Footer /></>}>
      <NewArticleForm />
    </Suspense>
  );
}
...
function NewArticleForm() {
  ...
  if (status === "loading") {
    return (
      <>
        <Header />
        <main className="min-h-screen flex items-center justify-center text-muted">Loading...</main>
        <Footer />
      </>
    );
  }
      ...
      <>
        ...
        <main className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted">You need EDITOR role or above to create articles.</p>
          </div>
        </main>
        <Footer />
      </>
  ...
  const isApprover = session.user.role === "APPROVER" || session.user.role === "SUPER_ADMIN";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      const statusText = data.data.status === "APPROVED"
        ? "Article published successfully!"
        : "Article submitted for approval.";
      setMessage({ type: "success", text: `${statusText} Redirecting...` });
      setTimeout(() => {
        ...
        <div className="max-w-3xl mx-auto px-4 py-10">
          ...
          <p className="text-sm text-muted mb-6">
            {isApprover
              ...
              : "Article will be submitted for approval."}
          </p>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              >
                {submitting
                  ...
                  : (isApprover ? "Publish Article" : "Submit for Approval")}
              </button>
              <button
                type="button"
                onClick={() => router.back()}
